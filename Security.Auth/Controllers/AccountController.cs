...
namespace OnlineAuction.Security.Auth.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private readonly TestUserStore _userTestStore;

        public AccountController(
            IIdentityServerInteractionService interaction,
            IEventService events,
            AccountService accountService,
            TestUserStore userTestStore = null)
        {
            _interaction = interaction;
            _events = events;
            _account = accountService;
            _userTestStore = userTestStore;
        }
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            return await LoginTestUser(model);
        }

        private async Task<IActionResult> LoginTestUser(LoginInputModel model)
        {
            if (ModelState.IsValid)
            {
                // validate username/password against in-memory store
                if (_userTestStore.ValidateCredentials(model.Username, model.Password))
                {
                    var user = _userTestStore.FindByUsername(model.Username);
            ...
            if (AccountOptions.WindowsAuthenticationSchemeName == provider)
            {
                ...
                if (result?.Principal is WindowsPrincipal wp)
                {
                    ...
                    await HttpContext.SignInAsync(
                        Identity4.IdentityServerConstants.ExternalCookieAuthenticationScheme,
        ...
        public async Task<IActionResult> ExternalLoginCallback()
        {
            ...
            var result = await HttpContext.AuthenticateAsync(Identity4.IdentityServerConstants.ExternalCookieAuthenticationScheme);
            ...
            if (user == default(TestUser))
            {
                ...
                user = _userTestStore.AutoProvisionUser(provider, userId, claims);
            }
            ...
            await HttpContext.SignOutAsync(Identity4.IdentityServerConstants.ExternalCookieAuthenticationScheme);
